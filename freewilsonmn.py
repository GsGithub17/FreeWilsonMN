...
analysis_type = st.sidebar.radio("Select Analysis", ["Free-Wilson Analysis", "Molecule Networks", "Molecule Networks with Structres"])
...
if analysis_type == "Free-Wilson Analysis":
    ...
    with st.expander("2. Molecule Preparation and R-Group Decomposition"):
        ...
        if core_mol is None:
            ...
            st.stop()
        # Compute 2D coordinates for the core molecule if not already present
        ...
        # RGroupDecompose returns a tuple (rgroups, unmatched)
        ...
        # Construct core DataFrame for later visualization (using the core molecule)
...
# ========================================
    ...
    **Concept:**
    ...
    # Use matplotlib to create a color map based on pIC50 values
    import matplotlib.colors as mcolors
    import matplotlib.cm as cm
    norm = mcolors.Normalize(vmin=df['pIC50'].min(), vmax=df['pIC50'].max())
    cmap = cm.get_cmap("viridis")

    # Build PyVis network graph with nodes colored by pIC50
    ...
    # Generate a colorbar image using matplotlib
...
elif analysis_type == "Molecule Networks with Structres":

    # with st.expander("Molecule Networks Visualization with 2D Structure Titles"):
    # st.markdown("""
    # **Concept:**
    # Compute the Morgan fingerprint for each molecule and calculate pairwise Tanimoto similarities.
    # An edge is drawn between two molecules if their similarity exceeds a user-defined threshold.
    # The resulting network is rendered interactively using PyVis.
    # In this visualization, each node’s title displays the 2D structure image along with its pIC50 value.
    # """)
    st.header("Molecule Networks Visualization with Structures")
    st.markdown("""
    **Concept:**
    Compute the Morgan fingerprint for each molecule and calculate pairwise Tanimoto similarities.
    An edge is drawn between two molecules if their similarity exceeds a user-defined threshold.
    ...
    # Build PyVis network with nodes colored by pIC50 and with structure images.
    nt = Network(height='600px', width=1000, notebook=False, cdn_resources="in_line")

    for idx, row in df.iterrows():
        # Generate the molecule image from SMILES
        mol = Chem.MolFromSmiles(row['SMILES'])
        mol_image = generate_molecule_image(mol, size=(200,200))
        # Use the pIC50 value as the label (as a string)
        score_string = str(row['pIC50'])
        # Get the node color from the colormap based on pIC50
        pIC50_val = row['pIC50']
        rgba = cmap(norm(pIC50_val))
        hex_color = mcolors.to_hex(rgba)
        # Add node with circular image, where the image is the molecule structure
        nt.add_node(idx,
                    label=score_string,
                    title=str(idx),
                    shape="circularImage",
    ...
    nt.show_buttons(filter_=['physics'])
    nt.save_graph("molecule_pIC50_network.html")

    # Display the network in Streamlit
    with open("molecule_pIC50_network.html", 'r', encoding='utf-8') as f:
        html_str = f.read()
    components.html(html_str, height=1500, width=1000)

    # Generate and display a colorbar for pIC50 using matplotlib
    fig, ax = plt.subplots(figsize=(4, 0.5))
    fig.subplots_adjust(bottom=0.5)
    smap_obj = cm.ScalarMappable(norm=norm, cmap=cmap)
    smap_obj.set_array([])
